...
def convert_to_array(input_str_data):
    ...
    img_array = np.array([int(sum_str_array(item)) for item in img_str_array],dtype=np.uint8)
...
def save_image(img_array, name, postfix="_decoded",format = ".jpg", path="decoded images"):
